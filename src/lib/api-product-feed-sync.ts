...
import { Prisma, type XmlFeedMatchBy, type XmlFeedPriceRound, type ApiProductFeed } from "@prisma/client";
...
async function loadRunResume(runId: string) {
  const rows = await prisma.$queryRawUnsafe<Array<{ resumeAt: number; seenIdsJson: string | null }>>(
    "SELECT `cursor` AS resumeAt, seenIdsJson FROM api_product_feed_runs WHERE id = ?",
    runId,
  );
  return {
...
async function writeRunProgress(
  ...
  if (data.seenIdsJson != null) {
    await prisma.$executeRawUnsafe(
      "UPDATE api_product_feed_runs SET itemCount = ?, createdCount = ?, updatedCount = ?, skippedCount = ?, failedCount = ?, `cursor` = ?, seenIdsJson = ? WHERE id = ?",
      data.itemCount,
      data.createdCount,
      data.updatedCount,
      data.skippedCount,
  ...
  await prisma.$executeRawUnsafe(
    "UPDATE api_product_feed_runs SET itemCount = ?, createdCount = ?, updatedCount = ?, skippedCount = ?, failedCount = ?, `cursor` = ? WHERE id = ?",
    data.itemCount,
    data.createdCount,
    data.updatedCount,
    data.skippedCount,
...
export async function syncApiFeedRun(runId: string) {
  const run = await prisma.apiProductFeedRun.findUnique({
    where: { id: runId },
    include: { feed: true },
  });
  if (!run) return;
  if (!run.feed.isActive) {
    ...
    await prisma.apiProductFeedRun.update({
      where: { id: runId },
      data: {
        status: "COMPLETED",
  ...
  await prisma.apiProductFeed.update({
    ...
    data: {
      lastStatus: "RUNNING",
      lastMessage: resuming ? "Senkron kaldığı yerden devam ediyor…" : "API okunuyor…",
    },
  });
  ...
  try {
    const fetched = await fetchApiFeedItems(feed.url, feed.itemPath, {
      httpMethod: feed.httpMethod,
      authType: feed.authType,
      httpUser: feed.httpUser,
      httpPass: feed.httpPass,
      request: parseApiFeedRequestJson(feed.requestJson),
    });
    const items = fetched.items;
    const itemPath = fetched.itemPath || feed.itemPath.trim();
    counts.items = items.length;
    await yieldToEventLoop();
    if (items.length === 0) {
      throw new Error("Ürün listesi bulunamadı. JSON yolunu seçin veya API yanıtını kontrol edin.");
    }

    if (itemPath && feed.itemPath.trim() !== itemPath) {
      await prisma.apiProductFeed.update({
        where: { id: feed.id },
        data: { itemPath },
      });
        ...
        if (prepared.length > 0) {
          const inserted = await insertImportedProductsSafely(
            ...
            (item, rowError) => {
              ...
              counts.failed += 1;
              pushFailure(failures, source ?? { rowNumber: 0, title: item.product.title }, shortenPrismaError(rowError));
            },
            { stockAsSupplier: advancedInventory },
          );
    ...
    const finishedAt = new Date();
    const latestMap = await prisma.apiProductFeed.findUnique({
      where: { id: feed.id },
