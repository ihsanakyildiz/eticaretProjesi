...
import { deleteFaqGroupAction, toggleFaqGroupActiveAction } from "./actions";
      ...
      >
        <div className="flex items-start gap-3 border-b border-[#e9ebec] px-5 py-4">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="text-base font-semibold text-slate-800">SSS grubunu sil</h2>
...
export function FaqGroupsTable({ groups }: { groups: FaqGroupRow[] }) {
  const router = useRouter();
  const [query, setQuery] = useState("");
  const [deleteTarget, setDeleteTarget] = useState<FaqGroupRow | null>(null);
  ...
  if (groups.length === 0) {
    return (
      <div className="rounded-lg border border-[#e9ebec] bg-white px-5 py-12 text-center shadow-sm">
        <p className="text-sm text-slate-500">Henüz SSS grubu yok.</p>
  ...
  return (
    <>
      <div className="rounded-lg border border-[#e9ebec] bg-white p-4 shadow-sm">
        <label htmlFor="faqs-search" className="mb-1.5 block text-xs font-medium text-slate-500">
          Arama
        </label>
              ...
              {filtered.map((group) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-end gap-1.5">
                    <Can resource="faqs" action="update">
                      <form action={toggleFaqGroupActiveAction}>
                    ...
                    <Can resource="faqs" action="delete">
                      <button
                        type="button"
                        onClick={() => {
                          setDeleteError(null);
        ...
        <DeleteGroupModal
          ...
          onConfirm={() => {
            ...
            startTransition(async () => {
              const result = await deleteFaqGroupAction(formData);
              if (result.error) {
