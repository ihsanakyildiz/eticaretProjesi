...
import {
  createProductFilterValueAction,
  updateProductFilterValueAction,
  type ProductFilterFormState,
} from "./actions";

const initialState: ProductFilterFormState = {};

export type FilterValueFormValues = {
  id?: string;
  name?: string;
  slug?: string;
...
export function ProductFilterValueForm({
  mode,
  filterId,
  inputType,
  initial,
  ...
  initial?: FilterValueFormValues;
  onSuccess: () => void;
  onCancel: () => void;
}) {
  const action =
    mode === "create" ? createProductFilterValueAction : updateProductFilterValueAction;
  const [state, formAction, isPending] = useActionState(action, initialState);
  const [name, setName] = useState(initial?.name ?? "");
  const [slug, setSlug] = useState(initial?.slug ?? "");
  const [slugTouched, setSlugTouched] = useState(Boolean(initial?.slug));
  const [colorHex, setColorHex] = useState(initial?.colorHex ?? "#111827");
      ...
      <div className="grid gap-4 p-5 md:grid-cols-2">
        <div className="md:col-span-2">
          ...
          <input
            ...
            placeholder={inputType === "SWATCH" ? "Örn. Siyah" : "Örn. Pamuk"}
            className={inputClass}
          />
        </div>
        <div>
