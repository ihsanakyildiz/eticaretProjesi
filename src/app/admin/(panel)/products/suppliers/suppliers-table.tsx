      ...
      >
        <div className="flex items-start gap-3 border-b border-[#e9ebec] px-5 py-4">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="text-base font-semibold text-slate-800">Tedarikçiyi sil</h2>
        ...
        <div className="space-y-3 px-5 py-4">
          <p className="text-sm leading-relaxed text-slate-600">
            <strong className="font-semibold text-slate-800">{supplier.name}</strong> tedarikçisini
            silmek istediğinize emin misiniz?
          </p>
          <ul className="list-disc space-y-1 pl-5 text-sm text-slate-500">
            <li>Tedarikçi kalıcı olarak silinecek.</li>
            {supplier._count.products > 0 ? (
              <li>
                {supplier._count.products} üründen tedarikçi bağlantısı kaldırılacak (ürünler
                silinmez).
              </li>
            ) : null}
            {supplier.logo ? <li>Logo sunucudan kaldırılacak.</li> : null}
          </ul>
          {error ? (
...
export function SuppliersTable({ suppliers }: { suppliers: SupplierRow[] }) {
  ...
  const confirmDelete = () => {
    ...
    startTransition(async () => {
      const result = await deleteSupplierAction({ id: deleteTarget.id });
      if (result.error) {
  ...
  const toggleActive = (supplier: SupplierRow) => {
    startTransition(async () => {
      const result = await toggleSupplierActiveAction({
        id: supplier.id,
      ...
      <div className="rounded-lg border border-[#e9ebec] bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-[#e9ebec] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <p className="text-xs text-slate-400">
            {filtered.length} / {suppliers.length} tedarikçi
          </p>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="px-4 py-12 text-center">
            <p className="text-sm text-slate-500">
              {suppliers.length === 0
                ...
                : "Aramanızla eşleşen tedarikçi yok."}
            </p>
            {suppliers.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[860px]">
              <div className="grid grid-cols-[minmax(0,1.6fr)_minmax(0,0.8fr)_minmax(0,0.9fr)_70px_80px_90px_150px] gap-2 border-b border-[#e9ebec] bg-[#f3f6f9] px-4 py-2.5 text-xs font-semibold tracking-wide text-slate-500 uppercase">
                <span>Firma</span>
                <span>Şehir</span>
                <span>Telefon</span>
                <span>Sıra</span>
              ...
              {filtered.map((supplier) => (
                <div
                  key={supplier.id}
                  className="grid grid-cols-[minmax(0,1.6fr)_minmax(0,0.8fr)_minmax(0,0.9fr)_70px_80px_90px_150px] items-center gap-2 border-b border-[#e9ebec] px-4 py-3 text-sm last:border-0"
                >
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="truncate font-medium text-slate-800">{supplier.name}</p>
                      <p className="truncate text-xs text-slate-400">
                        {supplier.email || supplier.slug}
                      </p>
                    </div>
                  </div>
                  <span className="truncate text-slate-500">{supplier.city || "—"}</span>
                  <span className="truncate text-slate-500">{supplier.phone || "—"}</span>
                  <span className="text-slate-500">{supplier.sortOrder}</span>
                  <span className="text-slate-500">{supplier._count.products}</span>
                  <span>
                    {supplier.isActive ? (
                  ...
                  <div className="flex items-center justify-end gap-1">
                    <Can resource="suppliers" action="update">
                      <Link
                        href={`/admin/products/suppliers/${supplier.id}/edit`}
                    ...
                    <Can resource="suppliers" action="update">
                      <button
                        ...
                        title={supplier.isActive ? "Pasife al" : "Aktif et"}
                      >
