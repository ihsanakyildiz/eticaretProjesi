...
import {
  activateBlogCategoryAction,
  deactivateBlogCategoryAction,
...
type TreeRow = CategoryTreeNode<BlogCategoryRow>;

type DeleteModalState =
  | { type: "confirm"; category: BlogCategoryRow }
  | { type: "blocked"; category: BlogCategoryRow }
  | null;
...
function CategoryTreeRows({
  ...
  onRequestDelete: (category: BlogCategoryRow) => void;
  onRequestDeactivate: (category: BlogCategoryRow) => void;
  onRequestActivate: (category: BlogCategoryRow) => void;
}) {
  return (
    <>
      {nodes.map((category) => {
          ...
          <div key={category.id}>
            ...
            >
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="truncate font-medium text-slate-800">
                      <AdminPublicTextLink href={publicBlogCategoryHref(category.slug)}>
                        {category.name}
                      </AdminPublicTextLink>
                    </p>
                    {category.depth > 0 ? (
              ...
              <div>
                ...
                >
                  {publicBlogCategoryHref(category.slug)}
                </AdminPublicTextLink>
              </div>
              ...
              <div className="flex items-center justify-end gap-1.5">
                <AdminPublicLink href={publicBlogCategoryHref(category.slug)} />
                <Can resource="blog_categories" action="create">
                  <Link
                    href={`/admin/blog/categories/new?parentId=${category.id}`}
                ...
                <Can resource="blog_categories" action="update">
                  <button
                    type="button"
                ...
                <Can resource="blog_categories" action="delete">
                  <button
                    type="button"
                    title="Sil"
...
export function BlogCategoriesTable({
  ...
  const [deactivateTarget, setDeactivateTarget] = useState<BlogCategoryRow | null>(null);
  ...
  const requestDelete = (category: BlogCategoryRow) => {
    setActionError(null);
  ...
  const confirmDelete = (postsMode?: "delete" | "deactivate") => {
    if (!deleteModal || deleteModal.type !== "confirm") return;
    if (deleteModal.category._count.posts > 0 && !postsMode) {
      setActionError("Lütfen bağlı yazılar için bir seçenek belirleyin.");
      return;
    }
  ...
  const confirmDeactivate = (moves: Record<string, string>) => {
    ...
    startTransition(async () => {
      const result = await deactivateBlogCategoryAction({
  ...
  const requestActivate = (category: BlogCategoryRow) => {
    startTransition(async () => {
      await activateBlogCategoryAction(category.id);
      router.refresh();
    });
  };
  ...
  if (categories.length === 0) {
    return (
      <div className="rounded-lg border border-[#e9ebec] bg-white px-5 py-12 text-center shadow-sm">
        <p className="text-sm text-slate-500">Henüz blog kategorisi eklenmedi.</p>
        <Can resource="blog_categories" action="create">
          <Link
