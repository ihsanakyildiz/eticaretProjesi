...
function PostsMoveList({
  ...
  onMoveChange: (postId: string, categoryId: string) => void;
      ...
      {posts.map((post) => (
        ...
        >
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-slate-400">{post.isActive ? "Aktif" : "Pasif"}</p>
          </div>
          <select
            value={moves[post.id] ?? ""}
            onChange={(e) => onMoveChange(post.id, e.target.value)}
...
export function DeactivateCategoryModal({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setLoadError(null);
    void getCategoryPostsAction(category.id).then((result) => {
      if (cancelled) return;
  ...
  const remainingCount = posts.filter((post) => !moves[post.id]).length;

  return (
    <ModalShell
      title="Kategoriyi pasife al"
      subtitle="Bağlı yazılar da etkilenecek."
      tone="teal"
      wide
      isPending={isPending}
      onClose={onClose}
    ...
    >
      <div className="space-y-4">
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          ...
          yazılar da otomatik olarak pasife alınır.
        </div>

        {category._count.posts > 0 ? (
          <>
            <p className="text-sm text-slate-600">
              Aşağıdaki yazıları başka bir kategoriye taşıyabilirsiniz. Taşımazsanız pasife
              alınırlar.
            </p>
            <PostsMoveList
              posts={posts}
              ...
              onMoveChange={(postId, categoryId) =>
                setMoves((prev) => {
                  const next = { ...prev };
                  if (!categoryId) delete next[postId];
                  else next[postId] = categoryId;
                  return next;
                })
              }
            />
            {!loading && posts.length > 0 ? (
              <p className="text-xs text-slate-500">
                {remainingCount} yazı mevcut kategoride kalıp pasife alınacak.
              </p>
            ) : null}
          </>
        ) : (
          <p className="text-sm text-slate-600">
            Bu kategoride yazı yok. Yalnızca kategori pasife alınacak.
          </p>
        )}
...
export function DeleteCategoryImpactModal({
  ...
  error?: string | null;
  onClose: () => void;
  onConfirm: (postsMode?: "delete" | "deactivate") => void;
}) {
  const [posts, setPosts] = useState<CategoryPostItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [postsMode, setPostsMode] = useState<"delete" | "deactivate" | null>(null);

  const hasPosts = category._count.posts > 0;

  useEffect(() => {
    if (blocked || !hasPosts) return;
    let cancelled = false;
    setLoading(true);
    void getCategoryPostsAction(category.id).then((result) => {
      if (cancelled) return;
    ...
    <ModalShell
      ...
      subtitle={
        blocked
          ...
          : hasPosts
            ? "Bu kategoriye bağlı yazılar var."
            : "Bu işlem geri alınamaz."
      }
      tone={blocked ? "amber" : "rose"}
      wide={hasPosts && !blocked}
      isPending={isPending}
      onClose={onClose}
      footer={
        <>
          ...
          {!blocked ? (
            <button
              type="button"
              onClick={() => onConfirm(hasPosts ? postsMode ?? undefined : undefined)}
              disabled={isPending || (hasPosts && !postsMode)}
        ...
        {blocked ? (
          ...
          <>
            <p className="text-sm text-slate-600">
              ...
              bağlı <strong>{category._count.posts}</strong> yazı var. Ne yapmak istersiniz?
            </p>
            ...
            <div className="space-y-2">
              ...
              >
                <input
                  type="radio"
                  name="postsMode"
                  checked={postsMode === "delete"}
                  onChange={() => setPostsMode("delete")}
                  className="mt-1"
                />
                <span>
                  <span className="block text-sm font-semibold text-slate-800">
                    Evet, yazıları da sil
                  </span>
                  <span className="mt-0.5 block text-xs text-slate-500">
                    Bu kategoriye ekli tüm yazılar kalıcı olarak silinir.
                  </span>
                </span>
              </label>
              ...
              >
                <input
                  type="radio"
                  name="postsMode"
                  checked={postsMode === "deactivate"}
                  onChange={() => setPostsMode("deactivate")}
                  className="mt-1"
                />
                <span>
                  <span className="block text-sm font-semibold text-slate-800">
                    Hayır, yazıları silme
                  </span>
                  <span className="mt-0.5 block text-xs text-slate-500">
                    Yazılar pasife alınır, kategorisiz bırakılır ve yazılarda “Kategorisi
            ...
            <div>
              <p className="mb-2 text-xs font-semibold tracking-wide text-slate-500 uppercase">
                Bağlı yazılar ({posts.length || category._count.posts})
              </p>
              {loading ? (
                ...
                <ul className="admin-scroll-light max-h-60 space-y-1.5 overflow-y-auto rounded-lg border border-[#e9ebec] bg-[#f3f6f9] p-3 text-sm text-slate-700">
                  {posts.map((post) => (
                    <li key={post.id} className="truncate border-b border-[#e9ebec]/80 pb-1.5 last:border-0 last:pb-0">
                      • {post.title}
                    </li>
                  ))}
                </ul>
