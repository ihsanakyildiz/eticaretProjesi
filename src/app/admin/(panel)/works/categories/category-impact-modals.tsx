...
export type WorkCategoryRow = {
  ...
  isActive: boolean;
  sortOrder: number;
  _count: { works: number; children: number };
};
...
function WorksMoveList({
  ...
  works: CategoryWorkItem[];
  loading: boolean;
  moves: Record<string, string>;
  onMoveChange: (workId: string, categoryId: string) => void;
  ...
  if (works.length === 0) {
    return (
      <p className="rounded-lg border border-[#e9ebec] bg-[#f3f6f9] px-4 py-3 text-sm text-slate-500">
        Bu kategoride bağlı çalışma yok.
      </p>
    );
  }
  ...
  return (
    <div className="admin-scroll-light max-h-72 space-y-2 overflow-y-auto rounded-lg border border-[#e9ebec] bg-[#f3f6f9] p-2 pr-1">
      {works.map((work) => (
        ...
        >
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-medium text-slate-800">{work.title}</p>
            <p className="text-xs text-slate-400">{work.isActive ? "Aktif" : "Pasif"}</p>
          </div>
          <select
            value={moves[work.id] ?? ""}
            onChange={(e) => onMoveChange(work.id, e.target.value)}
...
export function DeactivateCategoryModal({
  ...
  const [works, setWorks] = useState<CategoryWorkItem[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setLoadError(null);
    void getCategoryWorksAction(category.id).then((result) => {
      if (cancelled) return;
  ...
  const remainingCount = works.filter((work) => !moves[work.id]).length;

  return (
    <ModalShell
      title="Kategoriyi pasife al"
      subtitle="Bağlı çalışmalar da etkilenecek."
      tone="teal"
      wide
      isPending={isPending}
      onClose={onClose}
    ...
    >
      <div className="space-y-4">
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          ...
          çalışmalar da otomatik olarak pasife alınır.
        </div>

        {category._count.works > 0 ? (
          <>
            <p className="text-sm text-slate-600">
              Aşağıdaki çalışmaları başka bir kategoriye taşıyabilirsiniz. Taşımazsanız pasife
              alınırlar.
            </p>
            <WorksMoveList
              works={works}
              ...
              onMoveChange={(workId, categoryId) =>
                setMoves((prev) => {
                  const next = { ...prev };
                  if (!categoryId) delete next[workId];
                  else next[workId] = categoryId;
                  return next;
                })
              }
            />
            {!loading && works.length > 0 ? (
              <p className="text-xs text-slate-500">
                {remainingCount} çalışma mevcut kategoride kalıp pasife alınacak.
              </p>
            ) : null}
          </>
        ) : (
          <p className="text-sm text-slate-600">
            Bu kategoride çalışma yok. Yalnızca kategori pasife alınacak.
          </p>
        )}
...
export function DeleteCategoryImpactModal({
  ...
  error?: string | null;
  onClose: () => void;
  onConfirm: (worksMode?: "delete" | "deactivate") => void;
}) {
  const [works, setWorks] = useState<CategoryWorkItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [worksMode, setWorksMode] = useState<"delete" | "deactivate" | null>(null);

  const hasWorks = category._count.works > 0;

  useEffect(() => {
    if (blocked || !hasWorks) return;
    let cancelled = false;
    setLoading(true);
    void getCategoryWorksAction(category.id).then((result) => {
      if (cancelled) return;
    ...
    <ModalShell
      ...
      subtitle={
        blocked
          ...
          : hasWorks
            ? "Bu kategoriye bağlı çalışmalar var."
            : "Bu işlem geri alınamaz."
      }
      tone={blocked ? "amber" : "rose"}
      wide={hasWorks && !blocked}
      isPending={isPending}
      onClose={onClose}
      footer={
        <>
          ...
          {!blocked ? (
            <button
              type="button"
              onClick={() => onConfirm(hasWorks ? worksMode ?? undefined : undefined)}
              disabled={isPending || (hasWorks && !worksMode)}
        ...
        {blocked ? (
          ...
          <>
            <p className="text-sm text-slate-600">
              ...
              bağlı <strong>{category._count.works}</strong> çalışma var. Ne yapmak istersiniz?
            </p>
            ...
            <div className="space-y-2">
              ...
              >
                <input
                  type="radio"
                  name="worksMode"
                  checked={worksMode === "delete"}
                  onChange={() => setWorksMode("delete")}
                  className="mt-1"
                />
                <span>
                  <span className="block text-sm font-semibold text-slate-800">
                    Evet, çalışmaları da sil
                  </span>
                  <span className="mt-0.5 block text-xs text-slate-500">
                    Bu kategoriye ekli tüm çalışmalar kalıcı olarak silinir.
                  </span>
                </span>
              </label>
              ...
              >
                <input
                  type="radio"
                  name="worksMode"
                  checked={worksMode === "deactivate"}
                  onChange={() => setWorksMode("deactivate")}
                  className="mt-1"
                />
                <span>
                  <span className="block text-sm font-semibold text-slate-800">
                    Hayır, çalışmaları silme
                  </span>
                  <span className="mt-0.5 block text-xs text-slate-500">
                    Çalışmalar pasife alınır, kategorisiz bırakılır ve çalışmalarda “Kategorisi
            ...
            <div>
              <p className="mb-2 text-xs font-semibold tracking-wide text-slate-500 uppercase">
                Bağlı çalışmalar ({works.length || category._count.works})
              </p>
              {loading ? (
                ...
                <ul className="admin-scroll-light max-h-60 space-y-1.5 overflow-y-auto rounded-lg border border-[#e9ebec] bg-[#f3f6f9] p-3 text-sm text-slate-700">
                  {works.map((work) => (
                    <li key={work.id} className="truncate border-b border-[#e9ebec]/80 pb-1.5 last:border-0 last:pb-0">
                      • {work.title}
                    </li>
                  ))}
                </ul>
