...
import {
  activateProjectCategoryAction,
  deactivateProjectCategoryAction,
...
type TreeRow = CategoryTreeNode<ProjectCategoryRow>;

type DeleteModalState =
  | { type: "confirm"; category: ProjectCategoryRow }
  | { type: "blocked"; category: ProjectCategoryRow }
  | null;
...
function CategoryTreeRows({
  ...
  onRequestDelete: (category: ProjectCategoryRow) => void;
  onRequestDeactivate: (category: ProjectCategoryRow) => void;
  onRequestActivate: (category: ProjectCategoryRow) => void;
}) {
  return (
    <>
      {nodes.map((category) => {
          ...
          <div key={category.id}>
            ...
            >
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="truncate font-medium text-slate-800">
                      <AdminPublicTextLink href={publicProjectCategoryHref(category.slug)}>
                        {category.name}
                      </AdminPublicTextLink>
                    </p>
                    {category.depth > 0 ? (
              ...
              <div>
                ...
                >
                  {publicProjectCategoryHref(category.slug)}
                </AdminPublicTextLink>
              </div>
              ...
              <div className="flex items-center justify-end gap-1.5">
                <AdminPublicLink href={publicProjectCategoryHref(category.slug)} />
                <Can resource="project_categories" action="create">
                  <Link
                    href={`/admin/projects/categories/new?parentId=${category.id}`}
                ...
                <Can resource="project_categories" action="update">
                  <button
                    type="button"
                ...
                <Can resource="project_categories" action="delete">
                  <button
                    type="button"
                    title="Sil"
...
export function ProjectCategoriesTable({
  ...
  const [deactivateTarget, setDeactivateTarget] = useState<ProjectCategoryRow | null>(null);
  ...
  const requestDelete = (category: ProjectCategoryRow) => {
    setActionError(null);
  ...
  const confirmDelete = (projectsMode?: "delete" | "deactivate") => {
    if (!deleteModal || deleteModal.type !== "confirm") return;
    if (deleteModal.category._count.projects > 0 && !projectsMode) {
      setActionError("Lütfen bağlı projeler için bir seçenek belirleyin.");
      return;
    }
  ...
  const confirmDeactivate = (moves: Record<string, string>) => {
    ...
    startTransition(async () => {
      const result = await deactivateProjectCategoryAction({
  ...
  const requestActivate = (category: ProjectCategoryRow) => {
    startTransition(async () => {
      await activateProjectCategoryAction(category.id);
      router.refresh();
    });
  };
  ...
  if (categories.length === 0) {
    return (
      <div className="rounded-lg border border-[#e9ebec] bg-white px-5 py-12 text-center shadow-sm">
        <p className="text-sm text-slate-500">Henüz proje kategorisi eklenmedi.</p>
        <Can resource="project_categories" action="create">
          <Link
