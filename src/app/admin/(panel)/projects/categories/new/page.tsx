...
export default async function NewProjectCategoryPage({ searchParams }: NewPageProps) {
    ...
    <div className="space-y-6">
      <div className="rounded-lg border border-[#e9ebec] bg-white p-5 shadow-sm">
        ...
        <h1 className="mt-1 text-xl font-semibold text-slate-800 sm:text-2xl">
          {selectedParent ? "Yeni Alt Kategori" : "Yeni Proje Kategorisi"}
        </h1>
        <p className="mt-2 text-sm text-slate-500">
          {selectedParent
            ? `Üst kategori: ${breadcrumb.map((item) => item.name).join(" › ")}`
            : "Örn. Kurumsal Web, Mobil Uygulama, Web Programlama — veya mevcut bir kategorinin altına ekleyin."}
        </p>
      </div>
