...
import { ProjectFeaturesTable } from "./features-table";

export const metadata: Metadata = {
  title: "Proje Özellikleri",
  description: "Projelerde seçilebilecek özellik ve teknolojileri yönetin",
};
...
export default async function ProjectFeaturesPage() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="mt-1 flex items-center gap-2 text-xl font-semibold text-slate-800 sm:text-2xl">
              <Sparkles className="h-6 w-6 text-[#405189]" />
              Proje Özellikleri
            </h1>
            <p className="mt-2 max-w-2xl text-sm text-slate-500">
              PHP, React, MySQL, WordPress gibi teknolojileri buraya ekleyin. Proje
              oluştururken bunlardan birden fazlasını seçebilirsiniz.
            </p>
          </div>
          <Link
      ...
      <ProjectFeaturesTable features={features} />
    </div>
  );
}
