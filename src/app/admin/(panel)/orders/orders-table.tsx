...
export function OrdersTable({ orders }: { orders: OrderRow[] }) {
  ...
  const filtered = useMemo(() => {
    return orders.filter((order) => {
      if (applied.readyOnly && !order.allItemsWarehouseReserved) return false;
  ...
  const readyCount = useMemo(
    () => orders.filter((order) => order.allItemsWarehouseReserved).length,
    [orders],
  );
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <button
          type="button"
          onClick={() => {
            setDraft((current) => ({ ...current, readyOnly: false }));
            setApplied((current) => ({ ...current, readyOnly: false }));
          }}
          className={
            !applied.readyOnly
              ? "rounded-md bg-slate-800 px-3 py-1.5 text-sm font-semibold text-white"
        ...
        <button
          type="button"
          onClick={() => {
            setDraft((current) => ({ ...current, readyOnly: true }));
            setApplied((current) => ({ ...current, readyOnly: true }));
          }}
          className={
            applied.readyOnly
              ? "rounded-md bg-emerald-700 px-3 py-1.5 text-sm font-semibold text-white"
              : "rounded-md border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-sm font-medium text-emerald-800 hover:bg-emerald-100"
          }
        >
          Kargoya hazır
          <span className="ml-1.5 tabular-nums opacity-90">
            ({readyCount.toLocaleString("tr-TR")})
          </span>
        </button>
      </div>
