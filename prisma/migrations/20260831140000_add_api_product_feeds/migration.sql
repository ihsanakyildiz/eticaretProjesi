-- CreateTable
CREATE TABLE `api_product_feeds` (
    ...
    `matchBy` ENUM('BARCODE', 'SKU', 'PRODUCT_CODE', 'PRODUCT_ID') NOT NULL DEFAULT 'BARCODE',
    `skuPrefix` VARCHAR(40) NOT NULL DEFAULT '',
    `httpUser` VARCHAR(191) NULL,
    `httpPass` TEXT NULL,
    `httpMethod` ENUM('GET', 'POST') NOT NULL DEFAULT 'GET',
    `authType` ENUM('NONE', 'BEARER', 'BASIC', 'HEADER', 'QUERY') NOT NULL DEFAULT 'NONE',
    ...
    INDEX `api_product_feeds_isActive_nextRunAt_idx`(`isActive`, `nextRunAt`),
    INDEX `api_product_feeds_supplierId_idx`(`supplierId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- CreateTable
CREATE TABLE `api_product_feed_runs` (
    `id` VARCHAR(191) NOT NULL,
    ...
    `seenIdsJson` TEXT NOT NULL DEFAULT ('[]'),
    `message` TEXT NULL,
    `startedAt` DATETIME(3) NULL,
    `finishedAt` DATETIME(3) NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),

    INDEX `api_product_feed_runs_feedId_createdAt_idx`(`feedId`, `createdAt`),
    INDEX `api_product_feed_runs_status_createdAt_idx`(`status`, `createdAt`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- AddForeignKey
ALTER TABLE `api_product_feeds`
    ADD CONSTRAINT `api_product_feeds_supplierId_fkey`
    FOREIGN KEY (`supplierId`) REFERENCES `suppliers`(`id`)
    ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `api_product_feed_runs`
    ADD CONSTRAINT `api_product_feed_runs_feedId_fkey`
    FOREIGN KEY (`feedId`) REFERENCES `api_product_feeds`(`id`)
